...
interface RetailerTypesToolbarProps {
  totalItems: number;
  viewMode: 'list' | 'grid';
...
export function RetailerTypesToolbar({
  totalItems,
  viewMode,
  onViewModeChange,
  onSearch,
  onStatusChange,
}: RetailerTypesToolbarProps) {
  ...
  return (
    <div className="flex items-center justify-end gap-4">
      <div className="flex gap-4">
        {/* Search */}
        {totalItems > DEFAULT_RETAILER_TYPE_PAGINATION.pageSize && (
          <div className="relative w-sm">
            <Search className="text-muted-foreground pointer-events-none absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2" />
            <Input
              placeholder={__('Search by name', 'retailers-management-for-woocommerce')}
              value={search}
        ...
        <Select value={status} onValueChange={handleStatusChange}>
          <SelectTrigger className="border-input bg-background text-muted-foreground h-10! w-[160px] rounded-sm px-3 text-sm shadow-xs">
            {status === 'all'
              ? __('All Statuses', 'retailers-management-for-woocommerce')
              : status === '1'
                ? __('Active', 'retailers-management-for-woocommerce')
                : __('Inactive', 'retailers-management-for-woocommerce')}
          </SelectTrigger>
          <SelectContent>
